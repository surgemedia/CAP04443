...
class WooCommerce_Show_Attributes {
	...
	*/
	public function show_atts_in_admin_order( $product, $item, $item_id ) {
		...
		if ( is_object($product) ) {
			echo '<td width="25%" ><div class="view">' . $this->the_attributes( $product, 'span', $show_weight, $show_dimensions, $skip_atts ) . '</div></td>';
		}
	}
	...
	*/
	public function admin_order_item_header() {
		...
		if ( get_option( 'wcsa_admin_order_details' ) != 'no' ) {
			...
			echo '<th colspan="1" class="wsa-custom-attributes">' . __( 'Attributes', 'woocommerce-show-attributes' ) . '</th>';
		}
	}
	...
	*/
	...
	public function email_style( $css ) {
		$css .= '.custom-attributes {display: block}.custom-attributes > span {display: block}.custom-attributes .attribute-label{ width:100% !Important; clear:both  !Important; display:block !Important; white-space: nowrap}.custom-attributes .attribute-value { clear:both; padding-left: 8px;display: table-cell}.custom-attributes br {display: none}';
		return $css;
	}
