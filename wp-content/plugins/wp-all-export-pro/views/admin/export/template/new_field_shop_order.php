				...
				<?php foreach (XmlExportWooCommerceOrder::$order_sections as $section_key => $section) : ?>
					...
					<optgroup label="<?php echo $section['title']; ?>">
						<?php foreach ($section['meta'] as $field_key => $field) : ?>						
							<option value="<?php echo $field_key; ?>" rel="<?php echo (is_array($field) and ! empty($field['options']) ) ? $field['options'] : $section_key; ?>"><?php echo (is_array($field)) ? $field['name'] : $field; ?></option>
						<?php endforeach; ?>
					</optgroup>
