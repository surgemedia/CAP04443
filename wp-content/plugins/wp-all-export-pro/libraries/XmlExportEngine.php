	...
	{		
		...
		public function __construct( $post, & $errors ){		
			...
			{ 
				...
				if ( in_array('comments', self::$post_types) ) self::$is_comment_export = true;
			...
			else
			{
				...
				if ( 'wp_comment_query' == $this->post['wp_query_selector'] ){
					self::$is_comment_export = true;
				}
			}			
			...
			$this->comment_export = XmlExportComment::getInstance();
		...
		protected function init(){
			...
			if ('advanced' == $this->post['export_type']) { 
				...
				if ( self::$is_user_export )
				{
					add_action('comments_clauses', 'wp_all_export_pre_user_query', 10, 1);
					...
					remove_action('comments_clauses', 'wp_all_export_pre_user_query');
				}
				elseif( self::$is_comment_export )
				{
		...
		public function init_available_data( ){
			...
			$this->comment_export->init($this->_existing_meta_keys);			
